...
public class RechargeActivity extends BaseActivity {
    ...
    private String dou;
    private String payType = "2";//1：微信；2：支付宝
    ...
    private void initData() {
        ...
        tv_money.setText(dou + "豆");
    }

    @OnClick(R.id.tv_ok)
    void tv_ok() {
        if (StringUtil.isEmpty(et_money.getEditableText().toString().trim())
                || "0".equals(et_money.getEditableText().toString().trim())
                || "0.0".equals(et_money.getEditableText().toString().trim())) {
            showToast("请输入充值的金额");
        } else {
            if (payType.equals("1")) {
    ...
    void ic_wx() {
        payType = "1";
        ic_alipay.setImageResource(R.drawable.ic_notcheck);
        ic_wx.setImageResource(R.drawable.ic_checked);
    }

    @OnClick(R.id.ic_alipay)
    void ic_alipay() {
        payType = "2";
        ic_wx.setImageResource(R.drawable.ic_notcheck);
        ic_alipay.setImageResource(R.drawable.ic_checked);
    }
...
//                        payNum = orderString.split("=")[1];
    ...
    private void paySuccess(String s) {
        ...
        params.put("money", et_money.getEditableText().toString().trim());
        params.put("type", s);
        Log.i("tag", Constants.BASE_URL + "rechangeMoney&" + params);
        client.post(Constants.BASE_URL + "rechangeMoney", params, new AsyncHttpResponseHandler() {
            @Override
    ...
    private void payWithAliPay() {
        // 订单
        String orderInfo = getOrderInfo("创业豆充值", "创业豆充值", "0.01");
